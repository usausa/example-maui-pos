...
public sealed class TransactionRulesTests
{
    ...
    public void ValidateSaleAccepts()
    {
        ...
        var claimed = SalesCalculator.Calculate(input);

        var validation = TransactionRules.ValidateSale(SaleContext(), input, claimed);

        Assert.True(validation.IsValid);
        Assert.Empty(validation.Warnings);
        Assert.NotNull(validation.Expected);
        Assert.True(SalesResultComparer.Matches(claimed, validation.Expected));
    }
    ...
    public void ValidateSaleShift()
    {
        ...
        var notFound = TransactionRules.ValidateSale(SaleContext(shiftMissing: true), input, claimed);
        Assert.Contains(notFound.Errors, static x => x.Code == ErrorCode.ShiftNotFound);

        var closed = TransactionRules.ValidateSale(SaleContext(shift: Shift(ShiftStatus.Closed)), input, claimed);
        Assert.Contains(closed.Errors, static x => x.Code == ErrorCode.ShiftClosed);

        var mismatch = TransactionRules.ValidateSale(SaleContext(shift: Shift(ShiftStatus.Open, OtherTerminalId)), input, claimed);
    ...
    public void ValidateSaleDuplicateReceiptNo()
    {
        ...
        var validation = TransactionRules.ValidateSale(SaleContext(receiptNoInUse: true), input, claimed);
    ...
    public void ValidateSaleProductNotFound()
    {
        ...
        var validation = TransactionRules.ValidateSale(context, input, claimed);
    ...
    public void ValidateSalePriceOverrideNotAllowed()
    {
        ...
        var denied = TransactionRules.ValidateSale(SaleContext(), input, claimed);
        ...
        var allowed = TransactionRules.ValidateSale(SaleContext(products: [Product(SalesExample.CameraProduct, allowsPriceOverride: true), Product(SalesExample.SdCardProduct), Product(SalesExample.DeliveryProduct)]), input, claimed);
    ...
    public void ValidateSaleLineDiscountExceeded()
    {
        ...
        var claimed = SalesCalculator.Calculate(SalesExample.Input());
    ...
    {
        var input = SalesExample.Input();
        var claimed = SalesCalculator.Calculate(input) with { Total = 80000m };
    ...
    public void ValidateSaleChangeMismatch()
    {
        ...
        var input = SalesExample.Input() with { Payments = payments };
        var validation = TransactionRules.ValidateSale(SaleContext(), input, SalesCalculator.Calculate(input));
        var error = Assert.Single(validation.Errors);
        Assert.Equal(ErrorCode.PaymentMismatch, error.Code);
        ...
        validation = TransactionRules.ValidateSale(SaleContext(), input, SalesCalculator.Calculate(input));
    ...
    public void ValidateSaleCustomerRequired()
    {
        ...
        var validation = TransactionRules.ValidateSale(SaleContext(hasCustomer: false), input, claimed);
    ...
    public void ValidateReturnAccepts()
    {
        ...
        var claimed = ReturnCalculator.Calculate(input);

        var validation = TransactionRules.ValidateReturn(ReturnContext(), input, claimed);
    ...
    public void ValidateReturnOriginal()
    {
        ...
        var notFound = TransactionRules.ValidateReturn(ReturnContext(originalMissing: true), input, claimed);
        Assert.Contains(notFound.Errors, static x => x.Code == ErrorCode.OriginalNotFound);

        var voided = TransactionRules.ValidateReturn(ReturnContext(original: Original(TransactionType.Sale, TransactionStatus.Voided)), input, claimed);
        Assert.Contains(voided.Errors, static x => x.Code == ErrorCode.OriginalNotReturnable);

        var returnOfReturn = TransactionRules.ValidateReturn(ReturnContext(original: Original(TransactionType.Return, TransactionStatus.Completed)), input, claimed);
    ...
    public void ValidateReturnQuantityExceeded()
    {
        ...
        var exceededByReturned = TransactionRules.ValidateReturn(ReturnContext(), ReturnInput(2m) with { OriginalLines = originals }, claimed);
    ...
    public void ValidateReturnUnknownOriginalLine()
    {
        ...
        var validation = TransactionRules.ValidateReturn(ReturnContext(), input, ReturnCalculator.Calculate(ReturnInput(1m)));
    ...
    public void ValidateReturnPaymentMismatch()
    {
        ...
        var validation = TransactionRules.ValidateReturn(ReturnContext(), cashOnly, ReturnCalculator.Calculate(cashOnly));
        var error = Assert.Single(validation.Errors);
        Assert.Equal(ErrorCode.PaymentMismatch, error.Code);
        ...
        validation = TransactionRules.ValidateReturn(ReturnContext(), withChange, ReturnCalculator.Calculate(withChange));
        Assert.Contains(validation.Errors, static x => x.Code == ErrorCode.PaymentMismatch);
    }
    ...
    public void ValidateReturnCustomerRequired()
    {
        ...
        var validation = TransactionRules.ValidateReturn(ReturnContext(hasCustomer: false), input, ReturnCalculator.Calculate(input));
    ...
    {
        var valid = TransactionRules.ValidateVoid(new VoidContext { Transaction = Transaction(), ShiftStatus = ShiftStatus.Open });
        Assert.True(valid.IsValid);

        var notFound = TransactionRules.ValidateVoid(new VoidContext { Transaction = null, ShiftStatus = ShiftStatus.Open });
        Assert.Contains(notFound.Errors, static x => x.Code == ErrorCode.NotFound);

        var voided = TransactionRules.ValidateVoid(new VoidContext { Transaction = Transaction(status: TransactionStatus.Voided), ShiftStatus = ShiftStatus.Open });
        Assert.Contains(voided.Errors, static x => x.Code == ErrorCode.ValidationError);

        var closed = TransactionRules.ValidateVoid(new VoidContext { Transaction = Transaction(), ShiftStatus = ShiftStatus.Closed });
        Assert.Contains(closed.Errors, static x => x.Code == ErrorCode.ShiftClosed);

        var hasReturns = TransactionRules.ValidateVoid(new VoidContext { Transaction = Transaction(hasReturns: true), ShiftStatus = ShiftStatus.Open });
        Assert.Contains(hasReturns.Errors, static x => x.Code == ErrorCode.HasReturns);

        var returnWithFlag = TransactionRules.ValidateVoid(new VoidContext { Transaction = Transaction(TransactionType.Return, hasReturns: true), ShiftStatus = ShiftStatus.Open });
