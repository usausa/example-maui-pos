...
public sealed class ApiTransactionFlowTests : IClassFixture<TestApplicationFactory>
{
    ...
    public async Task ShiftSaleReturnVoidCloseAndReports()
    {
        ...
        Assert.Equal(shiftId, (await client.GetJsonAsync<ShiftResponseItem>($"{ApiRoutes.Shifts}/current?terminalId={InitialData.MainTerminal1Id}", options)).Id);
        ...
        var changes = await client.GetJsonAsync<InventoryChangeResponse>($"{ApiRoutes.Inventory}/changes?storeId={InitialData.MainStoreId}&type=Sale", options);
        Assert.Equal(2, changes.Total);
        Assert.All(changes.Items, x => Assert.Equal(sale.Id, x.ReferenceId));
        Assert.Equal(1, (await client.GetJsonAsync<TerminalResponseItem>($"{ApiRoutes.Terminals}/{InitialData.MainTerminal1Id}", options)).LastReceiptSeq);
        ...
        Assert.Equal(1, (await client.GetJsonAsync<ShiftResponse>($"{ApiRoutes.Shifts}?storeId={InitialData.MainStoreId}&status=Closed&from=2026-09-11&to=2026-09-11", options)).Total);
    ...
    public async Task InventoryChangesAreIdempotent()
    {
        ...
        var levels = await client.GetJsonAsync<InventoryLevelResponse>($"{ApiRoutes.Inventory}?storeId={InitialData.BranchStoreId}&productId={InitialData.SdCardProductId}", options);
        Assert.Equal(before + 4m, Assert.Single(levels.Items).Quantity);
        var changes = await client.GetJsonAsync<InventoryChangeResponse>($"{ApiRoutes.Inventory}/changes?storeId={InitialData.BranchStoreId}&productId={InitialData.SdCardProductId}", options);
        Assert.Equal(2, changes.Total);
