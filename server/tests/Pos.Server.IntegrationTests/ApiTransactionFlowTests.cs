...
// 端末の 1 日: 開設 → 販売 (api-design §4.6 の例) → 再送 → 返品 → 取消 → 入出金 → 精算 → 精算レポート・売上レポート
public sealed class ApiTransactionFlowTests : IClassFixture<TestApplicationFactory>
{
    ...
    public async Task ShiftSaleReturnVoidCloseAndReports()
    {
        ...
        var open = new ShiftOpenRequest { Id = shiftId, StoreId = InitialData.MainStoreId, TerminalId = InitialData.MainTerminal1Id, BusinessDate = BusinessDate, OpenedAt = Now, OpenedByStaffId = InitialData.MainCashierStaffId, OpeningCash = 30000m };
        using var openResponse = await client.PostJsonAsync(ApiRoutes.Shifts, open, options);
        var shift = await openResponse.ReadAsAsync<ShiftResponse>(HttpStatusCode.Created, options);
        ...
        Assert.Equal(shiftId, (await reopenResponse.ReadAsAsync<ShiftResponse>(HttpStatusCode.OK, options)).Id);
        using var secondOpenResponse = await client.PostJsonAsync(ApiRoutes.Shifts, new ShiftOpenRequest { Id = Guid.NewGuid(), StoreId = InitialData.MainStoreId, TerminalId = InitialData.MainTerminal1Id, BusinessDate = BusinessDate, OpenedAt = Now, OpenedByStaffId = InitialData.MainCashierStaffId, OpeningCash = 0m }, options);
        await secondOpenResponse.ReadProblemAsync(HttpStatusCode.Conflict, "TERMINAL_HAS_OPEN_SHIFT", options);
        Assert.Equal(shiftId, (await client.GetJsonAsync<ShiftResponse>($"{ApiRoutes.Shifts}/current?terminalId={InitialData.MainTerminal1Id}", options)).Id);

        var cameraBefore = await QuantityAsync(client, InitialData.CameraProductId);
        var sdCardBefore = await QuantityAsync(client, InitialData.SdCardProductId);
        ...
        var saleResult = await saleResponse.ReadAsAsync<TransactionResponse>(HttpStatusCode.Created, options);
        ...
        Assert.Equal(sale.Id, (await resendResponse.ReadAsAsync<TransactionResponse>(HttpStatusCode.OK, options)).Id);
        ...
        Assert.Equal(8074, (await client.GetJsonAsync<CustomerResponse>($"{ApiRoutes.Customers}/{InitialData.Customer1Id}", options)).PointBalance);
        var history = await client.GetJsonAsync<PointHistoryListResponse>($"{ApiRoutes.Customers}/{InitialData.Customer1Id}/points/history", options);
        ...
        Assert.Equal(cameraBefore - 1m, await QuantityAsync(client, InitialData.CameraProductId));
        Assert.Equal(sdCardBefore - 2m, await QuantityAsync(client, InitialData.SdCardProductId));
        var changes = await client.GetJsonAsync<InventoryChangeListResponse>($"{ApiRoutes.Inventory}/changes?storeId={InitialData.MainStoreId}&type=Sale", options);
        Assert.Equal(2, changes.Total);
        Assert.All(changes.Items, x => Assert.Equal(sale.Id, x.ReferenceId));
        Assert.Equal(1, (await client.GetJsonAsync<TerminalResponse>($"{ApiRoutes.Terminals}/{InitialData.MainTerminal1Id}", options)).LastReceiptSeq);

        // 返品 (SD カード 1 枚): §4.5 の計算
        ...
        var returnResult = await returnResponse.ReadAsAsync<TransactionResponse>(HttpStatusCode.Created, options);
        Assert.Equal(TransactionType.Return, returnResult.Type);
        Assert.Equal(8074 + 123 - 18, returnResult.PointsBalanceAfter);
        Assert.Equal(1m, (await client.GetJsonAsync<TransactionResponse>($"{ApiRoutes.Transactions}/{sale.Id}", options)).Lines[1].ReturnedQuantity);
        Assert.Equal(sdCardBefore - 1m, await QuantityAsync(client, InitialData.SdCardProductId));
        ...
        var voidRequest = new TransactionVoidRequest { StaffId = InitialData.ManagerStaffId, Reason = "誤操作", VoidedAt = Now.AddMinutes(30) };
        ...
        var voided = await voidReturnResponse.ReadAsAsync<TransactionResponse>(HttpStatusCode.OK, options);
        Assert.Equal(TransactionStatus.Voided, voided.Status);
        Assert.Equal("誤操作", voided.Void?.Reason);
        Assert.Equal(8074, (await client.GetJsonAsync<CustomerResponse>($"{ApiRoutes.Customers}/{InitialData.Customer1Id}", options)).PointBalance);
        Assert.Equal(0m, (await client.GetJsonAsync<TransactionResponse>($"{ApiRoutes.Transactions}/{sale.Id}", options)).Lines[1].ReturnedQuantity);
        Assert.Equal(sdCardBefore - 2m, await QuantityAsync(client, InitialData.SdCardProductId));
        ...
        // 一覧・検索
        Assert.Equal(2, (await client.GetJsonAsync<TransactionListResponse>($"{ApiRoutes.Transactions}?shiftId={shiftId}", options)).Total);
        Assert.Equal(1, (await client.GetJsonAsync<TransactionListResponse>($"{ApiRoutes.Transactions}?shiftId={shiftId}&status=Completed", options)).Total);
        Assert.Equal(sale.Id, (await client.GetJsonAsync<TransactionResponse>($"{ApiRoutes.Transactions}/lookup?receiptNo={sale.ReceiptNo}", options)).Id);
        Assert.Equal(2, (await client.GetJsonAsync<TransactionListResponse>($"{ApiRoutes.Customers}/{InitialData.Customer1Id}/transactions", options)).Total);
        ...
        await cashEventResponse.ReadAsAsync<CashEventResponse>(HttpStatusCode.Created, options);
        using var cashEventAgainResponse = await client.PostJsonAsync($"{ApiRoutes.Shifts}/{shiftId}/cash-events", cashEvent, options);
        await cashEventAgainResponse.ReadAsAsync<CashEventResponse>(HttpStatusCode.OK, options);
        Assert.Equal(1, (await client.GetJsonAsync<CashEventListResponse>($"{ApiRoutes.Shifts}/{shiftId}/cash-events", options)).Total);

        // 開設中の集計: 30000 + 25100 − 0 + 0 − 10000
        var opened = await client.GetJsonAsync<ShiftResponse>($"{ApiRoutes.Shifts}/{shiftId}", options);
        ...
        using var closeResponse = await client.PostJsonAsync($"{ApiRoutes.Shifts}/{shiftId}/close", new ShiftCloseRequest { ClosedAt = Now.AddHours(8), ClosedByStaffId = InitialData.MainCashierStaffId, ActualCash = 45000m, Denominations = [new ShiftCloseRequestDenomination { Denomination = 10000, Count = 4 }, new ShiftCloseRequestDenomination { Denomination = 5000, Count = 1 }] }, options);
        var closed = await closeResponse.ReadAsAsync<ShiftResponse>(HttpStatusCode.OK, options);
        ...
        using var noCurrentResponse = await client.GetAsync(new Uri($"{ApiRoutes.Shifts}/current?terminalId={InitialData.MainTerminal1Id}", UriKind.Relative), Token);
        ...
        Assert.Equal(1, (await client.GetJsonAsync<ShiftListResponse>($"{ApiRoutes.Shifts}?storeId={InitialData.MainStoreId}&status=Closed&from=2026-09-11&to=2026-09-11", options)).Total);

        // 帳票 PDF (D-37)
        await AssertPdfAsync(client, $"{ApiRoutes.Shifts}/{shiftId}/summary/pdf", "shift-report");
        await AssertPdfAsync(client, $"{ApiRoutes.Reports}/sales/daily/pdf?storeId={InitialData.MainStoreId}&date=2026-09-11", "daily-sales");
    ...
    public async Task InventoryChangesAreIdempotent()
    {
        ...
        var request = new InventoryChangeRequest
        {
            Changes =
            [
                new InventoryChangeRequestChange { Id = Guid.NewGuid(), StoreId = InitialData.BranchStoreId, ProductId = InitialData.SdCardProductId, Type = InventoryChangeType.PhysicalCount, Quantity = before + 5m, StaffId = InitialData.BranchCashierStaffId, OccurredAt = Now },
                new InventoryChangeRequestChange { Id = Guid.NewGuid(), StoreId = InitialData.BranchStoreId, ProductId = InitialData.SdCardProductId, Type = InventoryChangeType.Adjustment, Quantity = -1m, Reason = "破損", StaffId = InitialData.BranchCashierStaffId, OccurredAt = Now }
            ]
        };
        ...
        var levels = await client.GetJsonAsync<InventoryLevelListResponse>($"{ApiRoutes.Inventory}?storeId={InitialData.BranchStoreId}&productId={InitialData.SdCardProductId}", options);
        Assert.Equal(before + 4m, Assert.Single(levels.Items).Quantity);
        var changes = await client.GetJsonAsync<InventoryChangeListResponse>($"{ApiRoutes.Inventory}/changes?storeId={InitialData.BranchStoreId}&productId={InitialData.SdCardProductId}", options);
        Assert.Equal(2, changes.Total);
    ...
    // api-design §4.6: デジカメ (展示品 5%) + SD カード × 2 + 配送料、取引値引 1,000、ポイント 5,000 + カード 50,000 + 現金 25,100 (預り 30,000)
    private static TransactionRequest CreateSaleRequest(Guid shiftId)
    {
        ...
        return new TransactionRequest
        {
            ...
            StoreId = InitialData.MainStoreId,
            TerminalId = InitialData.MainTerminal1Id,
            StaffId = InitialData.MainCashierStaffId,
            ShiftId = shiftId,
            CustomerId = InitialData.Customer1Id,
    ...
    private static TransactionRequest CreateReturnRequest(Guid shiftId, Guid originalId, TransactionResponseLine original, decimal quantity, string receiptNo)
    {
