...
public sealed class InventoryReceiptFormValidator : FormValidator<InventoryReceiptForm>
{
    public InventoryReceiptFormValidator()
    {
        RuleFor(static x => x.StoreId).NotNull().WithMessage("店舗を選択してください。");
        RuleFor(static x => x.SupplierId).NotNull().WithMessage("仕入先を選択してください。");
        RuleFor(static x => x.SlipNo).MaximumLength(Length.SlipNo);
