...
public sealed class ProductFormValidator : FormValidator<ProductForm>
{
    public ProductFormValidator()
    {
        ...
        RuleFor(static x => x.Barcode).MaximumLength(Length.Barcode);
        RuleFor(static x => x.Name).NotEmpty().WithMessage("商品名を入力してください。").MaximumLength(Length.Name);
        RuleFor(static x => x.Kana).MaximumLength(Length.Kana);
        RuleFor(static x => x.Brand).MaximumLength(Length.Brand);
        RuleFor(static x => x.ModelNo).MaximumLength(Length.ModelNo);
        ...
        RuleFor(static x => x.Unit).MaximumLength(Length.Unit);
    }
}
