...
public static class ApiRuleText
{
    public static string Of(RuleReason reason) => reason switch
    {
        ...
        RuleReason.DepositMethodInvalid => "前受金は現金・カード・QR 決済・電子マネーで受け取ってください",
        RuleReason.DepositNotFound => "前受金がありません",
        RuleReason.DepositHeld => "前受金があります。端末で前受金を返してからキャンセルしてください",
        RuleReason.DepositMismatch => "前受金は、受注の前受金の全額を会計で充ててください",
        ...
        RuleReason.StaffNotAllowed => "担当は、その店舗 (または本部) の有効なスタッフにしてください",
        _ => reason.ToString()
    };
