...
// マスタ差分同期と Outbox 送信 (api-design §6)。バックグラウンドで定期実行し、書き込み直後は Trigger で早める
public sealed class SyncWorker : IDisposable
{
    ...
    private readonly ILogger<SyncWorker> log;
...
#pragma warning restore CA1031
    ...
    public async ValueTask<ApiResult<SyncMastersResponse>> SyncMastersAsync(bool full, IProgress<string>? progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await accessor.UpsertSyncStateAsync(ServerTimeKey, response.ServerTime.ToString("O", CultureInfo.InvariantCulture));
            await accessor.UpsertSyncStateAsync(InventorySyncKey, response.ServerTime.ToString("O", CultureInfo.InvariantCulture));
