...
public sealed class SalesUsecase
{
    ...
    public SalesResult Calculate(SalesCart cart, IEnumerable<CartPayment> payments) =>
        SalesLogic.Calculate(TransactionMapper.ToSalesInput(cart, payments, session.TaxRounding, session.PointBasis));

    public IReadOnlyList<RuleError> Validate(SalesCart cart, IEnumerable<CartPayment> payments) =>
        TransactionLogic.ValidateInput(TransactionMapper.ToSalesInput(cart, payments, session.TaxRounding, session.PointBasis));
