...
`DialogId` (ポップアップ) 一覧: `InputNumber` / `LineEdit` (P-13) / `Discount` (P-15) / `ReasonSelect` (在庫調整理由・返品理由) / `Denominations` (金種別入力)。  
...
| P-13 | 数量・単価は電卓 (`InputNumber`)、明細値引は定義済み / 任意額 / 任意率を `IDialog.SelectAsync` で選ぶ (`DiscountChooser`)。承認が必要な値引は店長 / 管理者を承認者に選ぶ |
| T-20 | 金額ショートカットは預り金に加算、[ちょうど] は残り全額。支払が済むと「残り」の行がお釣りになる |
| T-22 | プレビューは `ReceiptFormatter` の 32 桁テキストを SkiaSharp で桁位置描画した画像 (端末フォントが等幅でなくても崩れない)。共有は PNG |
