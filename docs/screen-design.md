...
| S-91 | 受注 | 一覧 + ダイアログ | フィルタ (受注日・店舗・状態 (既定は未完了)・種別・受注番号 / 宛名 / 電話)、`MudDataGrid` (受注番号・受注日時・店舗・種別と状態のチップ・お客様・電話・商品・金額 (前受金があれば下に小さく)・希望日)、[追加] で登録ダイアログ。行クリックで詳細のダイアログ (お客様・経過・明細・前受金) から [入荷] [変更] [受注をキャンセル] [会計した取引] (S-21)。前受金は見るだけ (受け取り・返すのは端末) で、前受金があるうちは [受注をキャンセル] を押せない。登録・変更のダイアログは店舗・担当・種別 (登録のみ)・会員 (選ぶと宛名と電話を補う)・宛名・電話・希望日・備考・明細 (商品を選ぶと単価は売価) | `GET/POST /orders`, `GET/PUT /orders/{id}`, `.../arrive`, `.../cancel` |
