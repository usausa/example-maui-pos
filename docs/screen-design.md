...
| T-10 | 販売 | 明細を組み立てる | 会員チップ (タップ → T-14)、明細リスト (タップ → P-13、左スワイプで削除)、小計・値引・合計、画面内ボタン [⋯] (取引値引 P-15 / 配送先 T-16 / 保留 T-17 / 受注にする T-94 / クリア)。受注から会計しているときは会員の下に受注番号 (前受金があれば金額も) の帯 | メニュー | スキャン | 検索 | 会計 | (ローカル計算) |
...
| T-20 | 会計 | 支払を受ける | 合計、ポイント利用 (残高表示)、支払済 / 残り、支払方法ボタン (文言は支払方法の `shortName`、なければ `name`。ポイントと前受金は出さない)、**埋め込みテンキー** + 金額ショートカット、[ちょうど]、支払リスト (複数可。受注の前受金は先頭で外せない) | 戻る | ポイント | 会員 | 確定 | `POST /transactions` (Outbox 経由) |
...
| T-51 | 精算 | シフト終了 | 集計 (現金売上・返金・入出金・前受金 (あるときだけ)・予想現金)、未送信が残っていれば警告、実査金額 (`InputNumber` or 金種別)、過不足 | 戻る | 金種入力 | 未送信 | 精算 | `GET /shifts/{id}`, `POST /shifts/{id}/close` |
...
| T-20 | 金額ショートカットは預り金に加算、[ちょうど] は残り全額。支払が済むと「残り」の行がお釣りになる。受注の前受金は、画面に入るたびにオンラインなら受注を読み直し (返金や他の端末の会計に追従する)、全額を先頭の支払に置き直す (✕ を出さない)。前受金が合計を超えるときと、前受金の支払方法が端末にないときは案内して T-10 に戻る |
...
| T-51 / T-52 | 予想現金はローカルの取引・入出金・前受金から (`ShiftSummaryCalculator`。前受金はサーバが受け付けた記録を端末に写したもの)。前受金の行は現金の前受金があるシフトだけ出す。精算レポートは未送信がなければサーバの集計、あれば端末の集計。[印刷] は T-22 と同じく未実装と知らせる |
...
| T-92 / T-93 | オンライン限定。取得中と取得できないときは結果の上に案内を重ねる。[会計へ] (引き渡し待ちのとき) は明細・会員・前受金をカートに入れて T-10 へ進む (レジ未開設なら案内。販売中の明細があれば置き換えるか確かめる)。単価は売価を変更できる商品だけ受注の単価にする。[操作] (未完了のとき) は `Select` シートで「前受金を受け取る」(前受金がないとき) か「前受金を返す」(あるとき) と「受注をキャンセル」。前受金の受取は支払方法 (現金・カード・QR・電子マネー) → 金額 (電卓。受注の金額まで) → 伝票番号 (要る支払方法だけ) → 確認、返金は全額を受け取った方法で返す (どちらもレジの開設が要る)。キャンセルの理由は `ReasonSelect` で、前受金があれば返してからキャンセルする |
...
| S-31 | シフト詳細 | ダイアログ | 精算レポート (現金 (前受金の受取・返金を含む)・支払方法別・税率別・部門別)、入出金一覧、金種別枚数、取引一覧へのリンク、[精算レポート PDF] | `GET /shifts/{id}`, `.../summary`, `.../summary/pdf`, `.../cash-events` |
...
| S-48 | 発注 | 一覧 + ダイアログ | フィルタ (希望納期・店舗・仕入先・状態 (既定は未完了))、`MudDataGrid` (発注番号・登録日時・希望納期・店舗・仕入先・状態のチップ・商品・金額)、[発注を作成] で登録ダイアログ (店舗・仕入先・希望納期・備考・明細 (在庫管理の商品。選ぶと仕入単価は原価))。行クリックで詳細のダイアログ (発注・経過 (登録・発注の日時と発注した人・入荷予定)・明細 (発注・入荷と差・仕入単価・金額)) から [変更] [発注] (確認のうえ入荷予定を作る) [発注をキャンセル] [入荷予定] (S-45 の詳細) [発注書 PDF] | `GET/POST /inventory/purchase-orders`, `GET/PUT .../{id}`, `.../order`, `.../cancel`, `.../pdf` |
...
| S-56 | 支払方法 | 一覧 + 編集ダイアログ | コード・名称・種別・釣銭・参照要・並び順・有効 (`Points` と `Deposit` はそれぞれ 1 件のみの制約を表示) | `/payment-methods` |
