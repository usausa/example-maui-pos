...
| 認証・端末登録 | 後回し (MVP は認証なし。template-maui-server を参考に追加し、認証に依存しない機能を先に作る) | [D-09](#d-09-認証端末登録-後回し), [D-62](#d-62-認証に依存しない機能を先に実装する) |
...
- 認証で変わる箇所は、ソースに `// 認証の導入時:` (razor は `@* 認証の導入時: … *@`) の注記を付け、何を変えるかを書く。  
  一覧は [implementation-plan.md](implementation-plan.md#phase-8-認証端末登録) の Phase 8 に置き、Phase 8 で注記を消していく
- 新しい endpoint は api-design の「用途」列で管理 / 端末を分けておく (Phase 8 で認可ポリシーを割り当てる元になる)
- 端末向けの通知 (Phase 13a) は端末トークンで接続を認証するので、Phase 8 のあとにする。  
...
- サーバを複数にすると、他のサーバの書き込みは届かない。  
  そのときは端末向けのハブと合わせて SignalR (バックプレーン) にする
