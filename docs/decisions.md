...
| 後回しの機能 | 外部向け Webhook、管理画面の MFA / パスキー、端末向けの通知 (SignalR)、他システムとの連携は作らない。発注と受注の前受金はサーバと端末の中で完結する形で作る | [D-72](#d-72-後回しにしていた機能の扱い-外部連携と端末向けの通知は作らない) |
| 発注 | 下書きを [発注] すると明細を写した入荷予定を作り、入荷予定の受領とキャンセルで発注も入荷済み・キャンセルになる。発注書は PDF で人が送る | [D-76](#d-76-発注-下書きを発注すると入荷予定を作り受領とキャンセルで状態を合わせる) |
| 受注の前受金 | 端末のシフトで受け取り (現金は予想現金に入る)、会計で全額を前受金の支払として充てる。キャンセルは返してから。控えは受注票 PDF で兼ねる | [D-77](#d-77-受注の前受金-シフトで受け取り会計で全額を充てキャンセルは返してから), [D-78](#d-78-受注票-受注の控えを-pdf-で出し前受金の預り証を兼ねる) |
...
**その後**: 実装と突き合わせて規則を直し、画面の規則 (`server-ui.md` / `terminal-ui.md`) と Accessor の規則 (`accessor.md`) を分けた ([D-75](#d-75-規則を実装に合わせて直し繰り返す手順を-skills-にする))。
...
### D-75. 規則を実装に合わせて直し、繰り返す手順を skills にする

`.claude/rules/` の変更をいったんそのまま取り込んでいたので、実装と突き合わせて見直した (利用者指示)。  
存在しない型や認証を入れる前のテストの書き方のような古い記述、同じ規則の重複 (`Length`、DB 操作での命名、記号)、規則にない規約 (認可の使い分け、API の形、端末の通信と Outbox、ポップアップ、DDL、テストの認証) が見つかった。  
検証、README の画像、エミュレータでの確認は、毎回その場のスクリプトで行っていた。

**決定**: 規則を実装に合わせて直し、確認できた規約はすべて足す (利用者指示)。  
繰り返す手順は skills にスクリプトと一緒に置く。

- 画面の規則は `server-ui.md` / `terminal-ui.md`、Accessor の規則は `accessor.md` に分ける (SQL の書き方の `sql.md` とは分ける。利用者指示)。  
  `server.md` の対象は `server/src` と `server/tools` に絞る (テストは `tests.md`)
- 判断を decisions.md に残すことと、レビューの指摘を規則に足すことは、コードを変えるときにも要るので `docs.md` から `AGENTS.md` に移す
- 文書の改行の規則を実際の書き方に合わせる (箇条書きの最後の文には「。」を付けない)
- skills は `verify` (ビルド、テスト、InspectCode、改行コード、文書の改行)、`readme-images` (README の画像)、`emulator` (エミュレータでの確認) の 3 つにする。  
  環境に固有の値 (機器、ポート) は書かずに引数で受け、エミュレータの操作は実機を選ばない
- 見直しで見つかったコードの不揃いは、Phase 8 で入れたものと不具合 2 件 (同じシフトの精算が同時に届いたときの 500、税率の既定の付け替えがトランザクションの外) を直し、それより前のものは触ったときに揃える

### D-76. 発注: 下書きを発注すると入荷予定を作り、受領とキャンセルで状態を合わせる

仕入先への注文を記録し、入荷予定と受領につなげる ([D-72](#d-72-後回しにしていた機能の扱い-外部連携と端末向けの通知は作らない) で残していた発注)。  
注文を仕入先に届ける部分 (EDI やメールでの送信) は作らないので、発注書は PDF で出して人が送る。

| 案 | 内容 |
| --- | --- |
| A. 入荷予定に「発注済み」の状態を足す | 表は増えないが、頼む側の伝票 (注文) と受け取る側の伝票 (入荷) の番号・日時・担当が 1 つの伝票に混ざる。既存の表に列も足す |
| ✅ **B. 発注を別の伝票にし、[発注] で入荷予定を作る** | 受領は今までどおり入荷予定で行う (管理画面と端末の検品)。発注は作った入荷予定を持ち、受領とキャンセルで状態を合わせる |

**決定**: ✅ **B**。

- 発注は下書き (`Draft`) → 発注済み (`Ordered`) → 入荷済み (`Received`)。  
  下書きと発注済みはキャンセル (`Cancelled`) できる。  
  変更は下書きの間だけ (版で楽観ロック。明細は置き換える)
- [発注] で、発注の明細 (数量・仕入単価) を写した入荷予定を作り、発注に `ReceiptId` を持つ。  
  1 つの発注に入荷予定は 1 つにし、分納や欠品は入荷予定の受領数と予定の差で残す
- 入荷予定を受領すると発注は入荷済みに、入荷予定をキャンセルすると発注もキャンセルになる。  
  発注をキャンセルすると入荷予定もキャンセルし、受領済みならキャンセルできない (どれも 1 つのトランザクション)
- 発注番号は店舗ごとの連番 (`{店舗コード}-P-{連番:000000}`)。  
  受注 (`-O-`) と移動 (`-T-`) と同じく 1 文の INSERT で採番する
- 入荷予定は発注から引いた発注番号を持つ。  
  端末の検品の一覧は、納品書番号がないときに発注番号を出す
- 発注した人は管理画面のアカウント名で持つ (日次締めの締めた人と同じ)
- 発注書は PDF (`OysterReport`) で、仕入先・発注元の店舗・希望納期・明細・合計を出す。  
  下書きとキャンセルは表題に状態を付ける
- 発注は管理画面だけで行う (API は `Policies.Admin`)。  
  端末は今までどおり入荷予定を検品する
- 在庫から発注の数を提案する機能 (発注点) は作らない (商品に発注点や仕入先の列を持たない)

### D-77. 受注の前受金: シフトで受け取り、会計で全額を充て、キャンセルは返してから

取り寄せ・取り置きで受け取る前受金 (内金) をどう持つか ([D-64](#d-64-受注-会計前の約束を別の資源で持ち会計で完了にする) では対象外にしていた)。  
前受金は会計の前にお金が動くので、受け取ったシフトの現金と、会計の売上・支払の両方に関わる。

| 案 | 内容 |
| --- | --- |
| A. 前受金を取引 (販売) として登録する | 売上が受け取ったときと会計のときの 2 回に分かれ、会計では前受金の分を値引や負の明細で打ち消す必要がある。売上の集計と返品の元取引が複雑になる |
| ✅ **B. 前受金を受注の記録にし、会計では支払の 1 つとして充てる** | `OrderDeposits` に受取と返金を持ち、会計では支払方法「前受金」(`Kind = Deposit`) で全額を充てる。売上は会計の 1 回だけで、取引の仕組みは変えない |

**決定**: ✅ **B**。

- 前受金は未完了の受注に 1 つだけにする (返したら受け取り直せる)。  
  金額は 1 円以上で受注の金額まで、支払方法は現金・カード・QR・電子マネー (ポイント・掛売・商品券は不可)
- 受け取るのも返すのも端末で、開設中のシフトに記録する (オンライン限定。取引と同じく、その端末のシフトで、自店の受注だけ)。  
  管理画面は見るだけにする (お金を扱わない画面で受け渡しを記録しない)
- 返金は前受金の全額を、受け取った方法で返す (一部の返金はしない)。  
  前受金があるうちは受注をキャンセルできず、端末のキャンセルは返してからキャンセルする
- 会計では、支払方法「前受金」の支払で前受金の全額を充てる。  
  端末の会計の画面は前受金を先頭の外せない支払にし、サーバは前受金の支払の合計が受注の前受金と同じことを確かめる (`PAYMENT_MISMATCH`)。  
  会計の合計が前受金より少ないときは会計できないので、前受金を返してから会計する (受注の変更で金額が下がっても変更は止めない)。  
  端末は会計の画面に入るときにオンラインなら受注を読み直し、保留の間に返された前受金や他の端末の会計に追従する
- 会計で受注が完了すると前受金は 0 になり、会計の取消で受注が引き渡し待ちに戻ると前受金も戻る (前受金の記録は消さず、会計で充てる額を「未完了なら受取 − 返金」で求める)
- 返品の返金に前受金は使えない (会計で充てた分も、返品では現金などで返す)
- 現金の前受金はシフトの予想現金に入れ (`DepositCashIn` / `DepositCashOut`)、精算で `Shifts` に確定する。  
  会計で充てた前受金は現金売上に入らない (受け取ったときに入っている)。  
  `Shifts` の列は起動時に足す (精算の集計列と同じく確定値を持つため。[実装計画](implementation-plan.md) の「既存テーブルへの列追加は避ける」の例外)
- 同時の操作は 1 文の条件付きの書き込みで守る。  
  受取・返金は受注が未完了で、前受金の残りが検証のときと同じで、シフトが開設中のときだけ INSERT し、キャンセルは残りが 0、会計の完了は残りが充てた額と同じときだけ UPDATE する
- 支払方法「前受金」は `Kind = Deposit` の有効な行を 1 件だけ持つ (ポイントと同じ)。  
  初期データに入れ、初期データより前の DB には起動時に足す
- 前受金の控え (預り証) は、管理画面から出す受注票 PDF で兼ねる ([D-78](#d-78-受注票-受注の控えを-pdf-で出し前受金の預り証を兼ねる))。  
  端末での印刷は [D-68](#d-68-端末の印刷は-bluetooth-ラインプリンタを前提にし今は作らない) で後回しにしている

### D-78. 受注票: 受注の控えを PDF で出し、前受金の預り証を兼ねる

取り寄せ・取り置きのお客様に渡す控えと、前受金の預り証をどう出すか ([D-77](#d-77-受注の前受金-シフトで受け取り会計で全額を充てキャンセルは返してから) では印刷しないとしていた)。  
端末の印刷は後回しなので ([D-68](#d-68-端末の印刷は-bluetooth-ラインプリンタを前提にし今は作らない))、サーバの帳票 ([D-37](#d-37-帳票出力-pdf-oysterreport)) で出す。

| 案 | 内容 |
| --- | --- |
| A. 前受金の預り証を別の帳票にする | 受取と返金のたびに 1 枚ずつ出せるが、受注の内容 (商品、希望日) が載らず、控えが 2 種類になる |
| ✅ **B. 受注票に前受金を載せ、預り証を兼ねる** | 受注の内容と前受金の受取・返金、お会計で差し引く額を 1 枚に載せる。前受金を受け取った後に出し直せば預り証になる |

**決定**: ✅ **B**。

- 受注票は管理画面の受注の詳細から PDF で出す (`GET /orders/{id}/pdf`、`Policies.Admin`)。  
  テンプレートは `Assets/Reports/Order.xlsx`
- 載せるのは、受注番号・受注日、お客様 (宛名・電話)、店舗 (会社・住所・電話)、種別・状態・希望日・担当・合計、明細、前受金 (受取と返金の記録、お会計で差し引く額)、案内、備考
- お客様に渡す控えなので、文言はお客様向けにする (「お会計で差し引く額」、取り置きは「お取り置きしております」)。  
  前受金がないときは前受金の欄を行ごと消す
- 完了とキャンセルは表題に状態を付け (「受注票 (お渡し済み)」「受注票 (キャンセル)」)、お渡し前の控えと区別する
