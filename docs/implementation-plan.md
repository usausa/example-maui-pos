...
実装順は Phase 9 → 10 → 11 → 12 → 13b → 14 → 8 (13a は作らない。[D-72](decisions.md#d-72-後回しにしていた機能の扱い-外部連携と端末向けの通知は作らない))。  
...
- [ ] 端末の印刷は Bluetooth ラインプリンタ (ESC/POS) を前提にし、今は作らない (利用者指示)。  
      OS の印刷 (`PrintManager`、「PDF に保存」) は使わない。  
      T-22 / T-52 の [印刷] は無効にせず、押すと「印刷は未実装です」と知らせる
...
13a (端末向けハブ) は作らない ([D-72](decisions.md#d-72-後回しにしていた機能の扱い-外部連携と端末向けの通知は作らない))。

### 13a 端末向けハブ (作らない)

- [ ] 端末向けの SignalR のハブは作らない (利用者指示)。  
      端末は書き込みの直後と 5 分ごとの差分同期でマスタと在庫に追い付き、端末の登録の解除は次の要求の 401 で知る (Phase 8)
...
- [x] 統合テスト (`ChangeNotificationTests`: 書き込みの通知・受け付けなかった書き込みは通知しない)、警告ゼロ、InspectCode ゼロ
...
取り寄せ・取り置きの前受金を端末のシフトで受け取り、会計で全額を充てる ([D-77](decisions.md#d-77-受注の前受金-シフトで受け取り会計で全額を充てキャンセルは返してから))。  
前受金は受注の記録にし、会計では支払方法「前受金」の支払にするので、売上は会計の 1 回だけのまま。

### 16a DB / API

- [x] `OrderDeposits` (受取 `Receive` / 返金 `Refund`、支払方法と種別、金額、端末・シフト・担当)。  
      会計で充てる額は未完了の受注の「受取 − 返金」
- [x] `POST /orders/{id}/deposit` (受取) と `POST /orders/{id}/deposit/refund` (全額を受け取った方法で返金)。  
      端末が id を決める登録なので、新規は 201、同じ id は 200、内容が違えば 409。  
      誤りは 422 `ORDER_DEPOSIT_INVALID` (新しいエラーコード)
- [x] 受注の応答に `depositAmount` と `deposits[]`。  
      前受金があるうちはキャンセルできない
- [x] 支払の種別 `Deposit` と支払方法「前受金」(初期データ。既存の DB には起動時に足す)。  
      `Kind = Deposit` の有効な行は 1 件だけ
- [x] 会計の検証: 前受金の支払の合計が受注の前受金と同じこと (`PAYMENT_MISMATCH`)、返品の返金に前受金を使えないこと。  
      会計の完了は前受金が検証のときと同じときだけ
- [x] シフトの集計に現金の前受金 (`depositCashIn` / `depositCashOut`) を足し、予想現金に入れる。  
      `Shifts` に確定の列を足す (起動時に `ALTER TABLE`)
- [x] `Pos.Domain`: `OrderDepositType`、`PaymentKind.CanReceiveDeposit`、`OrderLogic` (受取・返金・キャンセルの検証、会計で充てる額)、`TransactionLogic` (会計・返品の前受金)

### 16b 管理画面

- [x] 受注 S-91: 一覧の金額の下に前受金、詳細に前受金の記録と会計で充てる額 (見るだけ)。  
      前受金があるうちは [受注をキャンセル] を押せない
- [x] シフト詳細 S-31 と精算レポート PDF に前受金の受取・返金 (現金)
- [x] 支払方法 S-56: 種別「前受金」、1 件だけ有効にできる案内

### 16c 端末

- [x] 受注詳細 T-93: 前受金の表示、F3 [操作] (前受金を受け取る / 前受金を返す / 受注をキャンセル。前受金があれば返してからキャンセル)
- [x] 会計 T-20: 受注の前受金を先頭の外せない支払にする。  
      前受金が合計を超えるときは会計に進まない。  
      販売 T-10 の受注の帯に前受金、返金 T-42 の方法に前受金を出さない
- [x] サーバが受け付けた前受金を端末の `OrderDeposits` に写し、精算 T-51 と精算レポート T-52 の予想現金に入れる

### 16d テスト・docs

- [x] 統合テスト `ApiOrderDepositTests` (受取と再送、会計で充当、取消で戻る、返金とキャンセル、精算の予想現金、誤りの拒否、支払方法の 1 件の制約)、単体テスト (`OrderLogicTests`、`TransactionLogicTests`、`ErrorCodeTests`)
- [x] docs: api-design (§3.9、§3.12、§3.13、§3.15、§5)、db-design (ER、§3.1、§3.3、§3.6、§5、§6)、screen-design (T-10、T-20、T-42、T-51、T-93、S-31、S-56、S-91)、architecture、decisions (D-77)、README (端末の受注詳細の画像)

### 完了条件

- [x] 端末で受注に前受金を受け取り、会計で充てて完了し、キャンセルでは返してからキャンセルできる。  
      精算の予想現金に現金の前受金が入る
- [x] 統合テスト緑、警告ゼロ、InspectCode の指摘ゼロ
